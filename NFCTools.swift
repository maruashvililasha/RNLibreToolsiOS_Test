    ...
    func execute(_ taskRequest: TaskRequest) async throws {
        ...
        case .dump:
            ...
            do {
                ...
                print(data.hexDump(header: "ISO 15693 FRAM blocks:", startBlock: start))
                sensor.fram = Data(data)
                if sensor.encryptedFram.count > 0 && sensor.fram.count >= 344 {
                    print("\(sensor.fram.hexDump(header: "Decrypted FRAM:", startBlock: 0))")
                }
            } catch {
            }
            ...
            do {
                ...
                print(data.hexDump(header: "\'\(command)' command output (\(blocks) blocks):", startBlock: start))
...
//                }
                ...
                print("NFC: 'read blocks \(command)' command error: \(error.localizedDescription) (ISO 15693 error 0x\(error.iso15693Code.hex): \(error.iso15693Description))")
            }
        ...
        case .reset:
            ...
            case .libre1:
                ...
                do {
                    ...
                    print(data.hexDump(header: "NFC: did reset FRAM:", startBlock: start))
                    sensor.fram = Data(data)
                } catch {
